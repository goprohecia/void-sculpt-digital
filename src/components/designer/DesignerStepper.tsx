import { DESIGNER_STEPS } from "@/data/mockDesignerData";
import { Check, FileText, Image, Layers, MessageCircle, PenTool, CheckCircle, Download } from "lucide-react";
import { cn } from "@/lib/utils";

const STEP_ICONS = [FileText, Image, Layers, MessageCircle, PenTool, CheckCircle, Download];

interface Props { currentStep: number; }

export function DesignerStepper({ currentStep }: Props) {
  return (
    <div className="flex items-center gap-1 w-full overflow-x-auto py-2">
      {DESIGNER_STEPS.map((step, idx) => {
