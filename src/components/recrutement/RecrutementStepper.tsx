...
import { RECRUTEMENT_STEPS } from "@/data/mockRecrutementData";

interface RecrutementStepperProps {
  currentStep: number;
  compact?: boolean;
}

export function RecrutementStepper({ currentStep, compact }: RecrutementStepperProps) {
  return (
    <div className={compact ? "flex items-center gap-1 flex-wrap" : "space-y-2"}>
      {RECRUTEMENT_STEPS.map((step, i) => {
        ...
        if (compact) {
          return (
            <div key={step} className="flex items-center gap-1">
              ...
              {i < RECRUTEMENT_STEPS.length - 1 && <span className="text-muted-foreground/30 mx-0.5">›</span>}
            </div>
          );
        }
        return (
