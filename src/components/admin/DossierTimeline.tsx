...
import { Check, Clock, ChevronUp, MessageSquare, Mail, Phone, Lock } from "lucide-react";
...
export function DossierTimeline({ dossierId, isAdmin = false, isEmployee = false, isEnterprise = false }: DossierTimelineProps) {
  ...
  const { isStarter, isBusiness, isEnterprise: subEnterprise } = useSubscription();
  const notifConfigRef = useRef<Record<string, StepNotificationConfig>>({});

  const canAdvance = isAdmin || (isEmployee && !isStarter);
  const canRevert = isAdmin;
  const canEditConfig = isAdmin && (isBusiness || subEnterprise || isEnterprise);
  const showConfigUpsell = isAdmin && isStarter;
  const canAddNotes = isAdmin || isEmployee;
          ...
          {showConfigUpsell && (
            <Tooltip>
              <TooltipTrigger asChild>
                <Button size="sm" variant="outline" className="h-8 text-xs gap-1 opacity-50 cursor-not-allowed" disabled>
                  <Lock className="h-3 w-3" /> Config étapes
                </Button>
              </TooltipTrigger>
              <TooltipContent>Personnalisation des étapes disponible en Business</TooltipContent>
            </Tooltip>
          )}
        </div>
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-1.5">
                {/* Advance button */}
                {expandedStep >= currentStep && (isAdmin || isEmployee) && (
                  canAdvance ? (
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <span>
                          <Button size="sm" variant="default" className="text-xs h-7 gap-1 opacity-50" disabled>
                            <Lock className="h-3 w-3" /> Valider
                          </Button>
                        </span>
                      </TooltipTrigger>
                      <TooltipContent>Disponible en offre Business</TooltipContent>
                    </Tooltip>
                  )
                )}
                {/* Revert — admin only */}
                {canRevert && expandedStep < currentStep && (
