...
import { Shield, Plus, Trash2, Save, ChevronRight, FolderOpen, CalendarDays, BarChart3, Settings, Sparkles, UserPlus, X } from "lucide-react";
import { usePermissions, type Role } from "@/hooks/use-permissions";
import { useCustomSpaces } from "@/hooks/use-custom-spaces";
import { toast } from "sonner";
import { RoleEmployeeSection } from "@/components/admin/RoleEmployeeSection";
...
export function RolesPermissionsSettings() {
  ...
  // Check if a role is linked to a custom space
  const getLinkedSpace = (roleId: string) => spaces.find((s) => s.role_id === roleId);
        ...
        <CardContent>
          <div className="flex gap-6">
            ...
            <div className="w-56 shrink-0 space-y-1">
              {roles.map((role) => {
                const linkedSpace = getLinkedSpace(role.id);
                return (
                  <button
                    key={role.id}
                  ...
                  >
                    <div className="flex flex-col items-start gap-0.5">
                      ...
                      <span className="text-[10px] text-muted-foreground">
                        {getEmployeeCount(role.id)} salarié{getEmployeeCount(role.id) > 1 ? "s" : ""}
                      </span>
                    </div>
            ...
            <div className="flex-1 min-w-0">
              {selectedRole ? (
                <div className="space-y-5">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {getLinkedSpace(selectedRole.id) && (
                  ...
                  {/* Linked space info */}
                  {(() => {
                    const linkedSpace = getLinkedSpace(selectedRole.id);
                    if (!linkedSpace) return null;
                    return (
                      <div className="flex items-center gap-2 p-3 rounded-lg bg-amber-500/5 border border-amber-500/20 text-sm">
                        <Sparkles className="h-4 w-4 text-amber-500 shrink-0" />
                        <span className="text-muted-foreground">
                          Lié à l'espace <span className="font-medium text-foreground">« {linkedSpace.name} »</span> · {linkedSpace.enabled_modules.length} modules activés
                        </span>
                      </div>
                    );
                  })()}
                  ...
                  <RoleEmployeeSection roleId={selectedRole.id} />
