        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center mb-16 max-w-3xl mx-auto">
            <ScrollReveal variant="fadeInUp">
              <p className="text-xs uppercase tracking-[0.3em] text-muted-foreground mb-6">Pourquoi MBA</p>
            </ScrollReveal>
          ...
          <StaggerContainer className="max-w-4xl mx-auto space-y-3" staggerDelay={0.08} delayStart={0.2}>
            ...
            <div className="grid grid-cols-2 gap-4 px-4 pb-2">
              <p className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Le problème</p>
              <p className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">La solution MBA</p>
            </div>
            {comparisons.map((row, i) => (
              <motion.div key={i} variants={staggerItemVariants}>
                <div className="grid grid-cols-2 gap-4 p-4 rounded-xl border border-white/5 bg-white/[0.02] hover:border-white/10 transition-all">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <span className="text-sm text-foreground/90">{row.solution}</span>
                  </div>
                </div>
              </motion.div>
            ))}
