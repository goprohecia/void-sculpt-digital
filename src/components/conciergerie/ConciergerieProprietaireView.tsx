  ...
  return (
    <motion.div className="space-y-6" variants={staggerContainer} initial="initial" animate="animate">
      ...
      <motion.div className="grid grid-cols-2 md:grid-cols-4 gap-4" variants={staggerItem}>
        <DashboardKPI icon={Home} title="Biens gérés" value={mesBiens.length} iconColor="blue" />
        <DashboardKPI icon={CalendarDays} title="Réservations" value={mesReservations.length} iconColor="emerald" />
        <DashboardKPI icon={TrendingUp} title="Taux occupation" value={`${revenus.tauxOccupationMoyen}%`} iconColor="amber" />
        <DashboardKPI icon={Euro} title="Solde net" value={`${revenus.soldeNet.toLocaleString()} €`} iconColor="violet" />
      </motion.div>
