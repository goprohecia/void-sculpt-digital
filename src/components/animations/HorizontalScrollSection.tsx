...
export function HorizontalScrollSection({
  ...
  const translateX = useTransform(scrollYProgress, [0, 1], [0, -(scrollWidth - containerWidth + 100)]);
  ...
  if (isMobile) {
    return <section className={`py-16 relative ${className}`}>
        <div className="container mx-auto px-4">
          {/* Header */}
          {(title || badge) && <div className="text-center mb-12">
              ...
              {subtitle && <ScrollReveal variant="fadeInUp" delay={0.2}>
  ...
  return <section ref={containerRef} className={`relative ${className}`} style={{
    height: scrollableHeight > 0 ? scrollableHeight : "100vh"
  }}>
      <div className="sticky top-0 h-screen overflow-hidden flex flex-col">
        ...
        <div className="flex-1 flex items-center overflow-hidden">
          <motion.div ref={scrollRef} style={{
          x: springX
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 w-48 h-1 bg-white/10 rounded-full overflow-hidden">
          <motion.div className="h-full bg-gradient-to-r from-neon-violet to-purple-500 rounded-full" style={{
...
export function HorizontalScrollItem({
  ...
  return <motion.div variants={staggerItemVariants} className={`flex-shrink-0 ${className}`} whileHover={{
    scale: 1.02,
    y: -5
  }} transition={{
    duration: 0.3,
