...
import { FolderOpen, FileText, Receipt, MessageSquare, ArrowRight } from "lucide-react";
      ...
      <AdminPageTransition>
        <motion.div className="space-y-6" variants={staggerContainer} initial="initial" animate="animate">
          ...
          <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" variants={staggerContainer} initial="initial" animate="animate">
            <motion.div variants={staggerItem}>
              <DashboardKPI title="Dossiers en cours" value={dossiersEnCours} icon={FolderOpen} />
            </motion.div>
            <motion.div variants={staggerItem}>
              <DashboardKPI title="Devis en attente" value={devisEnAttente} icon={FileText} />
            </motion.div>
            <motion.div variants={staggerItem}>
              <DashboardKPI title="Factures à régler" value={facturesEnAttente} icon={Receipt} />
            </motion.div>
            <motion.div variants={staggerItem}>
              <DashboardKPI title="Messages non lus" value={messagesNonLus} icon={MessageSquare} />
            </motion.div>
          </motion.div>
          ...
          <motion.div className="glass-card p-4 sm:p-6" variants={staggerItem}>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/client/dossiers" className="text-xs text-primary hover:underline flex items-center gap-1">
          ...
          <motion.div className="glass-card p-4 sm:p-6" variants={staggerItem}>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/client/devis" className="text-xs text-primary hover:underline flex items-center gap-1">
                Voir tous <ArrowRight className="h-3 w-3" />
              </Link>
            </div>
            <div className="space-y-3">
          ...
          {messagesNonLus > 0 && (
            <motion.div variants={staggerItem}>
              <Link to="/client/messagerie" className="glass-card p-4 flex items-center gap-3 hover:border-[hsl(200,100%,50%)]/30 transition-colors block">
                <div className="rounded-xl bg-[hsl(200,100%,50%)]/10 p-2">
                  <MessageSquare className="h-5 w-5 text-[hsl(200,100%,60%)]" />
                </div>
