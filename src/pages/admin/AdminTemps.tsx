...
import { Timer, Play, Square, Clock, User, FolderOpen, TrendingUp } from "lucide-react";
...
export default function AdminTemps() {
  ...
  const totalMinutes = DEMO_ENTRIES.reduce((sum, e) => sum + e.dureeMin, 0);
  const totalHeures = Math.floor(totalMinutes / 60);
  const totalMin = totalMinutes % 60;
  const uniqueClients = new Set(DEMO_ENTRIES.map((e) => e.client)).size;
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            {[
              ...
              { label: "Entrées", value: DEMO_ENTRIES.length.toString(), icon: FolderOpen },
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center gap-4">
                ...
                <Badge variant="outline" className="text-xs">Aucun dossier sélectionné</Badge>
              </div>
            </CardContent>
          </Card>
